...
public class Agent implements Serializable, Runnable {
	...
	public void run( ) {
		...
		try{
			...
			if(_arguments == null){
				...
				method.invoke(this);
	System.out.println("agent run method without arguments");
			} else{
				Class[] arguments = new Class[1];
				arguments[0] = String[].class;
                method = this.getClass().getMethod(_function, arguments);
                // method = getClass().getMethod(_function,_arguments.getClass()) ;
                 method.invoke(this, (Object)_arguments);
                System.out.println("agent run method with arguments");
			}
		}catch(Exception e){
			//
		}
	...
	public void hop( String hostname, String function, String[] args ) {
		...
		_function = function;
		_arguments = args;
		_bytecode = getByteCode();     //byte code of the agent
		
		byte[] data = serialize();     //serialize the agent
		
		try{
			
			//invoke an RMI call.
			PlaceInterface place = (PlaceInterface)(Naming.lookup( "rmi://" + _hostname + ":" + _port + "/place" ));
			
			System.out.println("created the rmi call");
			Place  placemessage = null;
			
			System.out.println("arugements passed to the arraylist");
			placemessage.receiveMessage(_arguments);
//			Object[] message = placemessage.sendMessage();
//			System.out.println(message);
			//transfer to the next host
			System.out.println("transfer the agent to the next host");
			 place.transfer(_classname, _bytecode, data);
